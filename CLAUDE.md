...
equally fatal. Indirect calls behave per `cfg.indirect`: `warn` emits a blind-spot finding, `address-taken` fans every indirect site out to all address-taken defined nodes, `ignore` drops them. `Finding.key` deduplicates warnings; `_reconstruct` walks the BFS `prev` map, attributing each step the *call site* line rather than the callee's declaration line.
